...
serve_read(envid_t envid, union Fsipc *ipc)
{
    ...
    if ((r = openfile_lookup(envid, req->req_fileid, &open)) < 0)
        return r;
    if ((size = file_read(open->o_file, (void *) ret->ret_buf, req->req_n, open->o_fd->fd_offset)) < 0)
        return size;

    open->o_fd->fd_offset += size;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
    ...
    if ((r = openfile_lookup(envid, req->req_fileid, &open)) < 0)
        return r;
    if ((size = file_write(open->o_file, (void *) req->req_buf, req->req_n, open->o_fd->fd_offset)) < 0)
        return size;
