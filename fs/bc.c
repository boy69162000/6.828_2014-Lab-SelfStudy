...
bc_pgfault(struct UTrapframe *utf)
{
    ...
    if ((r = sys_page_alloc(0, ROUNDDOWN(addr, PGSIZE), PTE_SYSCALL)) < 0)
        panic("allocating page block addr %d, r=%e\n", addr, r);
    if ((r = ide_read(blockno*BLKSECTS, ROUNDDOWN(addr, PGSIZE), BLKSECTS)) < 0)
        panic("reading block %d, r=%e\n", blockno, r);
...
flush_block(void *addr)
{
    ...
    if (va_is_mapped(addr) && va_is_dirty(addr)) {
        if ((r = ide_write(blockno*BLKSECTS, ROUNDDOWN(addr, PGSIZE), BLKSECTS) < 0))
            panic("flush_block write failed va %08x, r=%e", addr, r);
        if ((r = sys_page_map(0, ROUNDDOWN(addr, PGSIZE), 0, ROUNDDOWN(addr, PGSIZE), PTE_SYSCALL)) < 0)
            panic("flush_block clear dirty failed va %08x, r=%e", addr, r);
    }
}
